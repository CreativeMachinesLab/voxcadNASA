...
void CVXS_Voxel::EulerStep(void)
{
	...
	if (IS_ALL_FIXED(DofFixed))
	{ //if fixed, just update the position and forces acting on it (for correct simulation-wide summing
	...
	else 
	{
		...
		if(p_Sim->IsMaxVelLimitEnabled())
		{ //check to make sure we're not going over the speed limit!
	...
	if (p_Sim->pEnv->EvolvingNeuralNetworksCheck())
	{
		// std::cout << "evolving neural nets" << std::endl;
		// Caclulate Electrical Activation:  

		if ( /*_pMat->GetNeuralActivatedMuscleType()*/ GetMuscleType()>0 )
		{	
			// std::cout << "muscle type: " << GetMuscleType() << std::endl;
			if (p_Sim->GetNeuralOutputOfIndex( /*_pMat->GetNeuralActivatedMuscleType()*/p_Sim->pEnv->pObj->GetNumSensors()+p_Sim->pEnv->pObj->GetNumHidden()+GetMuscleType()-1) > 0 ) //p_Sim->pEnv->GetActivationThreshold() )  // nac: add this as a parameter?
			{
				// std::cout << "index:" << MyXIndex << std::endl;
				...
				{
					// std::cout << "start depolarizing" << std::endl;
					S.RepolarizationStartTime = p_Sim->CurTime;
					// std::cout << "activated at: " << p_Sim->CurTime << std::endl;
				}
			}
			else
			{
		...
		// 	{
		// 		if (GetCurGroundPenetration()>0.0)
		// 		{
		// 			p_Sim->SetSensorValueOfIndex( /*_pMat->GetNeuralSensorType()*/ GetSensorType() -1, 1.0);
		// 		}
		// 		else
		// 		{
		// 			p_Sim->SetSensorValueOfIndex( /*_pMat->GetNeuralSensorType()*/ GetSensorType() -1, .0);
		// 		}
		// 	}
		// }
	if (false) //nac: for conductive, not neural
	{
		if ( _pMat->GetIsConductive() )
		{
			...
			{ 
				...
				else 
				{
					...
					{
						if ((p_Sim->CurTime - S.RepolarizationStartTime) < _pMat->GetRepolarizationTime()*4)
						{
							S.ElectricallyActiveNew = false;
						}
						else
	...
	if (_pMat->GetIsConductive())
	{
		...
		// if (MyXIndex ==0)
		// {
		// 	std::cout << "p_Sim->CurTime - S.RepolarizationStartTime: " << p_Sim->CurTime - S.RepolarizationStartTime << std::endl;
		// 	// std::cout << "S.RepolarizationStartTime: " << S.RepolarizationStartTime << std::endl;
		// 	std::cout << "p_Sim->pEnv->GetTempPeriod(): " << p_Sim->pEnv->GetTempPeriod() << std::endl;
		// }
		// if (p_Sim->pEnv->PhaseDrivenActuationCheck())
		// {
		// 	if (p_Sim->CurTime - S.RepolarizationStartTime <= p_Sim->pEnv->GetTempPeriod() )
		// 	{
		// 		// if (MyXIndex ==0){std::cout << 1+ p_Sim->pEnv->GetTempAmplitude()*GetCTE()*sin(2.0*3.1415926/ p_Sim->pEnv->GetTempPeriod() /* _pMat->GetRepolarizationTime()*/ * (p_Sim->CurTime - S.RepolarizationStartTime) ) << std::endl;}
		// 		// if (MyXIndex ==0){std::cout << p_Sim->pEnv->GetTempAmplitude() << std::endl;}
		// 		// if (MyXIndex ==0){std::cout << GetCTE() << std::endl;}
		// 		// if (MyXIndex ==0){std::cout << sin(2.0*3.1415926/ p_Sim->pEnv->GetTempPeriod() /* _pMat->GetRepolarizationTime()*/ * (p_Sim->CurTime - S.RepolarizationStartTime))  << std::endl;}
		// 		// if (MyXIndex ==0){std::cout << 1+ p_Sim->pEnv->GetTempAmplitude()*GetCTE()*sin(2.0*3.1415926/ p_Sim->pEnv->GetTempPeriod() /* _pMat->GetRepolarizationTime()*/ * (p_Sim->CurTime - S.RepolarizationStartTime) ) << std::endl;}
		// 		// if (MyXIndex ==0){std::cout << 1+ p_Sim->pEnv->GetTempAmplitude()*GetCTE()*sin(2.0*3.1415926/ p_Sim->pEnv->GetTempPeriod() /* _pMat->GetRepolarizationTime()*/ * (p_Sim->CurTime - S.RepolarizationStartTime) ) << std::endl;}
		// 		// if (MyXIndex ==0){std::cout << 1+ p_Sim->pEnv->GetTempAmplitude()*GetCTE()*sin(2.0*3.1415926/ p_Sim->pEnv->GetTempPeriod() /* _pMat->GetRepolarizationTime()*/ * (p_Sim->CurTime - S.RepolarizationStartTime) ) << std::endl;}

		// 		S.Scale = std::max(MIN_TEMP_FACTOR, 1+ p_Sim->pEnv->GetTempAmplitude()*GetCTE()*sin(2.0*3.1415926/ p_Sim->pEnv->GetTempPeriod() /* _pMat->GetRepolarizationTime()*/ * (p_Sim->CurTime - S.RepolarizationStartTime) ))*OriginalSize;
		// 		// SetColor(1,0,0,1);
		...
		// 		// if (MyXIndex == 5){std::cout << S.Scale.x << std::endl;}
		// 	}
		// 	else 
		// 	{
		...
		if (p_Sim->CurTime == 0)
		{
			SetTimeOfLastMuscleUpdate(-999);
			// S.Scale.x = OriginalSize.x;
		}

		if (p_Sim->CurTime - GetTimeOfLastMuscleUpdate() > 0.001)
		{
			// float maxUpdate = 0.01*GetCTE()*p_Sim->pEnv->GetTempPeriod()/(100*p_Sim->dt);
			// float maxUpdate = GetCTE()*(p_Sim->CurTime - timeOfLastMuscleUpdate)*pow(10,-4);
			float maxUpdate = 0.005*GetCTE();
			float oldSize = S.Scale.x;
			float newSize = (1+ p_Sim->pEnv->GetTempAmplitude()*GetCTE()*p_Sim->GetNeuralOutputOfIndex( /*_pMat->GetNeuralActivatedMuscleType()*/p_Sim->pEnv->pObj->GetNumSensors()+p_Sim->pEnv->pObj->GetNumHidden()+GetMuscleType()-1))*OriginalSize.x;
			float newSizeScaled = std::max(oldSize-maxUpdate,std::min(oldSize+maxUpdate,newSize));
			S.Scale = newSizeScaled*OriginalSize/OriginalSize.x;
			SetTimeOfLastMuscleUpdate(p_Sim->CurTime);
			
			// std::cout << "GetCTE(): " << GetCTE() << std::endl;
			// std::cout << "dt: " << p_Sim->dt << std::endl;
			// std::cout << "period: " << p_Sim->pEnv->GetTempPeriod() << std::endl;
			// if (MyXIndex == 999)
			// {
			// 	std::cout << "p_Sim->CurTime " << p_Sim->CurTime << std::endl;
			// 	std::cout << "p_Sim->pEnv->GetTempAmplitude() " << p_Sim->pEnv->GetTempAmplitude() << std::endl;
			// 	std::cout << "GetCTE() " << GetCTE() << std::endl;
			// 	// std::cout << "timeOfLastMuscleUpdate: " << GetTimeOfLastMuscleUpdate() << std::endl;
			// 	std::cout << "neuralUpdate: " << p_Sim->GetNeuralOutputOfIndex( /*_pMat->GetNeuralActivatedMuscleType()*/p_Sim->pEnv->pObj->GetNumSensors()+p_Sim->pEnv->pObj->GetNumHidden()+GetMuscleType()-1) << std::endl;
			// 	// std::cout << "OriginalSize.x " << OriginalSize.x << std::endl;
			// 	// std::cout << "maxUpdate " << maxUpdate << std::endl;
			// 	// std::cout << "S.Scale.x: " << S.Scale.x << std::endl;
		...
		// else if (MyXIndex == 999)
		// {
		// 	std::cout << "p_Sim->CurTime " << p_Sim->CurTime << std::endl;
		// 	std::cout << "timeOfLastMuscleUpdate: " << GetTimeOfLastMuscleUpdate() << std::endl;
		// 	std::cout << "p_Sim->CurTime - timeOfLastMuscleUpdate: " << p_Sim->CurTime - GetTimeOfLastMuscleUpdate() << std::endl;
		// 	if (p_Sim->CurTime - GetTimeOfLastMuscleUpdate() > 0.01)
		// 	{
		// 		std::cout << "p_Sim->CurTime - GetTimeOfLastMuscleUpdate() > 0.01: true"  << std::endl << std::endl;
		// 	}
		// 	else
		// 	{
		// 		std::cout << "p_Sim->CurTime - GetTimeOfLastMuscleUpdate() > 0.01: false"  << std::endl << std::endl;
		// 	}
		// }
	} 
	// 	else
	// 	{
	// 		//S.Scale = TempFact*OriginalSize;
	// 		// std::cout<<"not conductive"<<std::endl;
	// 		S.Scale = OriginalSize;
	// 		// if (S.Scale.x != 0.001)
	// 		// {
	...
	// else
	// {
	// 	// S.Scale = std::max( MIN_TEMP_FACTOR, p_Sim->pEnv->GetTempBase() + p_Sim->pEnv->GetTempAmplitude()*sin((2*3.1415926f/p_Sim->pEnv->GetTempPeriod()) * p_Sim->CurTime ))*OriginalSize;
	// 	// std::cout<<"not evolving neural nets"<<std::endl;
	// 	S.Scale = TempFact*OriginalSize;
	// }

	// if (MyXIndex ==0){std::cout << "S.Scale.x: " << S.Scale.x << std::endl;}
...
float CVXS_Voxel::UpdateElectricalActivationFromNeighbors(void)
{
	...
	for (int i=0; i<NumLocBond; i++){
		...
		if ( /*p_Sim->pEnv->pObj->GetBaseMat(*/pThisNeighborVoxel->/*GetMaterial())->GetIsGroundTouchSensor()*/GetSensorType()-1 and pThisNeighborVoxel->GetElectricallyActiveOld())  { S.ElectricallyActiveNew = true; S.RepolarizationStartTime = p_Sim->CurTime;}
...
vfloat CVXS_Voxel::GetCurGroundPenetration() //how far into the ground penetrating (penetration is positive, no penetration is zero)
{
	...
	if (false)
	{
		Penetration = (S.Scale.x+S.Scale.y+S.Scale.z)/6.0 - S.Pos.z - p_Sim->GetFloorHeight(S.Pos.x,S.Pos.y);
	}
	else
	{
		Penetration = (S.Scale.x+S.Scale.y+S.Scale.z)/6.0 - S.Pos.z;
	}
...
{
	return p_Sim->pEnv->pObj->GetMuscleTypeArray(MyXIndex);
}

int CVXS_Voxel::GetSensorType() 
{
	return p_Sim->pEnv->pObj->GetSensorTypeArray(MyXIndex);
}
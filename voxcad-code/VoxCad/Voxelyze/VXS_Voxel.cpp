...
void CVXS_Voxel::EulerStep(void)
{
	...
	if ( _pMat->GetNeuralActivatedMuscleType() )
	{	
		if (p_Sim->GetNeuralOutputOfIndex( _pMat->GetNeuralActivatedMuscleType()-1) > 0 ) //p_Sim->pEnv->GetActivationThreshold() )  // nac: add this as a parameter?
		{
	...
	{
		if ( _pMat->GetNeuralSensorType() )
		{
			p_Sim->SetSensorValueOfIndex( _pMat->GetNeuralSensorType()-1, GetCurGroundPenetration()>0);
		}
	}
	...
	if ( _pMat->GetIsConductive() )
	{
		...
		{ 
			if (_pMat->GetIsGroundTouchSensor())
			{
	...
	if (TempFact < MIN_TEMP_FACTOR) TempFact = MIN_TEMP_FACTOR;
	if (_pMat->GetIsConductive() or _pMat->GetNeuralActivatedMuscleType() )
	{
...
float CVXS_Voxel::UpdateElectricalActivationFromNeighbors(void)
{
	...
	for (int i=0; i<NumLocBond; i++){
		...
		if ( p_Sim->pEnv->pObj->GetBaseMat(pThisNeighborVoxel->GetMaterial())->GetIsGroundTouchSensor() and pThisNeighborVoxel->GetElectricallyActiveOld())  { S.ElectricallyActiveNew = true; S.RepolarizationStartTime = p_Sim->CurTime;}

...
CVXC_Material& CVXC_Material::operator=(const CVXC_Material& RefMat)
{
	...
	NeuralActivatedMuscleType = RefMat.NeuralActivatedMuscleType;
	NeuralSensorType = RefMat.NeuralSensorType;
	IsGroundTouchSensor = RefMat.IsGroundTouchSensor;
	IsPacemaker = RefMat.IsPacemaker;
	IsConductive = RefMat.IsConductive;
	RepolarizationTime = RefMat.RepolarizationTime;
	DepolarizationTime = RefMat.DepolarizationTime;
...
void CVXC_Material::ClearMaterial(void)
{
	...
	NeuralActivatedMuscleType = 0;
	NeuralSensorType = 0;
	IsGroundTouchSensor = false;
	IsPacemaker = false;
	IsConductive = false;
	RepolarizationTime = 0.0;
	DepolarizationTime = 0.0;
}
	...
	switch (MatType){
		case SINGLE:
			if (pXML->FindElement("Mechanical")){
				...
				if (!pXML->FindLoadElement("NeuralSensorType", &NeuralSensorType)) NeuralSensorType = 0;
				if (!pXML->FindLoadElement("NeuralActivatedMuscleType", &NeuralActivatedMuscleType)) NeuralActivatedMuscleType = 0;
				if (!pXML->FindLoadElement("IsGroundTouchSensor", &IsGroundTouchSensor)) IsGroundTouchSensor = false;
				if (!pXML->FindLoadElement("IsPacemaker", &IsPacemaker)) IsPacemaker = false;
				if (!pXML->FindLoadElement("IsConductive", &IsConductive)) IsConductive = false;
				if (!pXML->FindLoadElement("RepolarizationTime", &RepolarizationTime)) RepolarizationTime = 0.0;
				if (!pXML->FindLoadElement("DepolarizationTime", &DepolarizationTime)) DepolarizationTime = 0.0;
...
bool CVXC_Structure::ReadXML(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
{
	...
	if (Version == "" || Version == "0.93"){ //newest version
		...
		for (int i=0; i<Z_Voxels; i++){
			...
			else if (Compression == "LONG_READABLE") 
			{
				std::vector<std::string> RawDataVector = split(RawData, ",", false); 
				// DataIn.resize(RawDataVector.size());
				for (int k=0; k<(int)RawDataVector.size(); k++)
				{
					...
					SetData(X_Voxels*Y_Voxels*i + k, atoi(RawDataVector[k].c_str()) ); //pDataIn[k];
								
					// DataIn[i] = atoi(RawDataVector[i].c_str()); //if compressed using this scheme  //nac <-- DEBUG
					// strcpy(DataIn[i],RawDataVector[i].data()-48);
					// for (int j=0; j<RawDataVector[i].size(); j++)
					// {
					// 	DataIn += RawDataVector[i][j];
					// }
				}
				continue;
			}
			else if (Compression == "LONG_READABLE_2") 
			{
				// std::vector<std::string> RawDataVector = split(RawData, ",", false); 
				DataIn.resize(RawData.size());
				for (int i=0; i<(int)RawData.size(); i++)
				{
					DataIn[i] = RawData[i]-48; //if compressed using this scheme  //nac <-- DEBUG
				}
			}

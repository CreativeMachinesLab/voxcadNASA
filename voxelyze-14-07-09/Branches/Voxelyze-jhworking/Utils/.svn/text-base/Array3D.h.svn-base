...
//		  T& operator [](int i)       { return data[i]; }
  	const T& operator [](const index3D i3D) const { return data[GetIndex(i3D)];} //overload index operator
		  T& operator [](const index3D i3D) { return data[GetIndex(i3D)];}
	...
	const T& at(const index3D& i3D) const {
		int thisIndex = getIndex(i3D);
		if(thisIndex==-1) return defaultValue;
		else return data[thisIndex];
	} 

...
//	template <typename U> friend Vec3D<T> operator+(Vec3D<T> const& lhs, Vec3D<U> const& rhs){return Vec3D<T>(lhs.x+rhs.x, lhs.y+rhs.y, lhs.z+rhs.z);}
	...
	const T& operator[](int index) const {switch (index%3){case vec3_X: return x; case vec3_Y: return y; case vec3_Z: return z;}}
	T& operator[](int index) {switch (index%3){case vec3_X: return x; case vec3_Y: return y; case vec3_Z: return z;}}
...
//	CQuat(const CQuat& Quat) {*this = Quat;} //copy constructor
	CQuat(const Vec3D<T>& VecIn) { //constructs from a rotation vector
		//w = 0; x = VecIn.x; y = VecIn.y; z = VecIn.z;
		//adapted from http://physicsforgames.blogspot.com/2010/02/quaternions.html
		Vec3D<T> theta = VecIn/2;
		T s, thetaMag2 = theta.Length2();
		if (thetaMag2*thetaMag2 / 24.0 < DBL_EPSILON){ //if the 4th taylor expansion term is negligible
			w=1.0-thetaMag2 / 2.0;
			s=1.0-thetaMag2 / 6.0;
		}
		else {
	...
	CQuat(const T angle, const Vec3D<T> &axis){
		Clear();
...
//	T Angle() const {return 2.0*acos(w>1?1:w);}
	Vec3D<T> AxisNormalized() const {
	...
	Vec3D<T> Axis() const {return Vec3D<T>(x, y, z);}

...
//	CurSimFeatures |= VXSFEAT_IGNORE_ANG_DAMP;
...
bool CVX_Sim::Import(CVX_Environment* pEnvIn, CMesh* pSurfMeshIn, std::string* RetMessage)
{
	...
	//add the voxels
	Vx.setLatticeSize(LocalVXC.GetLatDimEnv().x); 
	int x, y, z;
	...
	for (int i=0; i<LocalVXC.GetStArraySize(); i++){ //for each voxel in the array
		...
		if (VxcMatIndex>=0){
			...
			VoxList.push_back(Vx.addVoxel(VxcToVx2MatIndexMap[VxcMatIndex], x, y, z));
		}
	}
	...
	for (int i=0; i<numVox; i++){
		...
		Vec3D<> ThisPos = pThisVox->position();
		for (int j = 0; j<NumBCs; j++){ //go through each primitive defined as a constraint!
			...
			if (pCurBc->GetRegion()->IsTouching(&ThisPos, &BCsize, &WSSize)){ //if this point is within
				if (IS_FIXED(DOF_X, ThisDofFixed)) pThisVox->setFixed(X_TRANSLATE, pCurBc->Displace.x);
				if (IS_FIXED(DOF_Y, ThisDofFixed)) pThisVox->setFixed(Y_TRANSLATE, pCurBc->Displace.y);
				if (IS_FIXED(DOF_Z, ThisDofFixed)) pThisVox->setFixed(Z_TRANSLATE, pCurBc->Displace.z);
				if (IS_FIXED(DOF_TX, ThisDofFixed)) pThisVox->setFixed(X_ROTATE, pCurBc->AngDisplace.x);
				if (IS_FIXED(DOF_TY, ThisDofFixed)) pThisVox->setFixed(Y_ROTATE, pCurBc->AngDisplace.y);
				if (IS_FIXED(DOF_TZ, ThisDofFixed)) pThisVox->setFixed(Z_ROTATE, pCurBc->AngDisplace.z);

				pThisVox->setExternalForce(pCurBc->Force/Sizes[j]);
				pThisVox->setExternalMoment(pCurBc->Torque/Sizes[j]);
			}
		}
	}
...
void CVX_Sim::CopyMat(CVXC_Material* pOld, CVX_Material* pNew) //copies parameters from pOld to pNew
{
	...
	case MDL_BILINEAR: pNew->setModelBilinear(pOld->GetElasticMod(), pOld->GetPlasticMod(), pOld->GetYieldStress()); break;
	case MDL_DATA: {

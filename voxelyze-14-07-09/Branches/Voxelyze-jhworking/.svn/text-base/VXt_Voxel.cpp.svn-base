...
CVXt_Voxel::CVXt_Voxel(CVX_Sim* pSimIn, int SIndexIn, int XIndexIn, int MatIndexIn, Vec3D<>& NominalPositionIn, vfloat OriginalScaleIn) 
{
	pSim = pSimIn;
...
CVXt_Voxel::~CVXt_Voxel(void)
{
}

CVXt_Voxel& CVXt_Voxel::operator=(const CVXt_Voxel& VIn)
{
...
Vec3D<> CVXt_Voxel::toGlobal(BondDir dirIn, const Vec3D<>& toTransform)
{
	switch (dirIn){
	case BD_PX: case BD_NX: return toTransform;
	case BD_PY: case BD_NY: return Vec3D<>(-toTransform.y, toTransform.x, toTransform.z);
	case BD_PZ: case BD_NZ: return Vec3D<>(-toTransform.z, toTransform.y, toTransform.x);
	default: return Vec3D<>(0,0,0);
	}
}


void CVXt_Voxel::SetExternalDisp(const Axis AxisIn, const vfloat DispIn) //Sets the displacement of this voxel in the specified direction
{
	switch (AxisIn){
...
void CVXt_Voxel::SetExternalTDisp(const Axis AxisIn, const vfloat TDispIn) //Sets the angular displacement of this voxel in the specified direction
{
	switch (AxisIn){
...
bool CVXt_Voxel::SetMaterial(const int MatIndexIn) {
	MatIndex = MatIndexIn;
...
bool CVXt_Voxel::LinkInternalBond(int SBondIndex, BondDir ThisBondDir) //simulation bond index...
{
...
void CVXt_Voxel::UpdateInternalBondPtrs() //updates all links (pointers) to bonds according to pSim
{
	for (int i=0; i<6; i++){
...
//CVXt_Voxel* CVXt_Voxel::pNearbyVox(int LocalNearbyInd) //To CVXS_Voxel?
//{
////	return &(p_Sim->VoxArray[NearbyVoxInds[LocalNearbyInd]]);
//	return (CVXt_Voxel*) &(p_Sim->VoxArray[NearbyVoxInds[LocalNearbyInd]]);
//
//}



void CVXt_Voxel::FixDof(char DofFixedIn) //fixes any of the degrees of freedom indicated. Doesn't unfix any currently fixed ones
{
...
void CVXt_Voxel::CalcNearby(CVX_Sim* pSim, int NumHops) //populates pNearbyVox
{
	NearbyVoxInds.clear();
	...
	for (int i=0; i<NumHops; i++){
		for (int j=StartPoint; j<StopPoint; j++){ //go through the list from the most recent interation...
			CVXt_Voxel* pThisVox = &(pSim->VoxArray[NearbyVoxInds[j]]);
...
//bool CVXt_Voxel::IsLinear() //returns true if the material model is linear
//{

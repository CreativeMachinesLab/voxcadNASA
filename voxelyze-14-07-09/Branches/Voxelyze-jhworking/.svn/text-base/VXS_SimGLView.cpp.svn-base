...
#ifdef VX2
	CVoxelyze* pVx = &pSim->Vx;
	const std::list<CVX_Link*>* links = pVx->linkList();
	for (std::list<CVX_Link*>::const_iterator it = links->begin(); it!=links->end(); it++){
		CVX_Link* pLink = *it;
		
		Vec3D<> P1 = pLink->voxel(false)->position();
		Vec3D<> P2 = pLink->voxel(true)->position();
		
		CColor ThisColor = CColor(0,0,0,1.0); //GetInternalBondColor(pBond);

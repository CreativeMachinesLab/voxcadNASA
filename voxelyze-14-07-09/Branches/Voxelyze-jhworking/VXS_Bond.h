...
//	vfloat GetEffectiveStiffness(void) const {return Eh*(CSArea1+CSArea2)/(2*(CurStrainTot*L.x+L.x));} //EA/L: accounting for volume effects
	vfloat GetEffectiveStiffness(void) const {return Eh*getCurrentAvgCSArea()/(CurStrainTot*L.x+L.x);} //EA/L: accounting for volume effects
...
//	vfloat CSArea;
	vfloat getCurrentAvgCSArea() const;

//	vfloat poissonStrainA[2], poissonStrainB[2]

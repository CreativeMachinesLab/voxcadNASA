...
vfloat CVXS_Bond::getCurrentAvgCSArea() const
{
	vfloat CSArea1, CSArea2;
	switch (ThisBondAxis){
	case AXIS_X: 
		CSArea1 = (1+pVox1->poissonsstrain.y)*(1+pVox1->poissonsstrain.z);
		CSArea2 = (1+pVox2->poissonsstrain.y)*(1+pVox2->poissonsstrain.z);
		break;
	case AXIS_Y: 
		CSArea1 = (1+pVox1->poissonsstrain.x)*(1+pVox1->poissonsstrain.z);
		CSArea2 = (1+pVox2->poissonsstrain.x)*(1+pVox2->poissonsstrain.z);
		break;
	case AXIS_Z: 
		CSArea1 = (1+pVox1->poissonsstrain.x)*(1+pVox1->poissonsstrain.y);
		CSArea2 = (1+pVox2->poissonsstrain.x)*(1+pVox2->poissonsstrain.y);
		break;
	}

	return 0.5*L.x*L.x*(CSArea1+CSArea2);

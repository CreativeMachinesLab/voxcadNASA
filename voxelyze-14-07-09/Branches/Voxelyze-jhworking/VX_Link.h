...
//links always point with voxel positive in the global positive direction (normalized on create)
class CVX_Link {
	public:
	CVX_Link(CVX_Voxel* voxel1, CVX_Voxel* voxel2, linkDirection direction); //!< Default Constructor
	~CVX_Link(void); //!< Destructor

	CVX_Voxel* voxel(bool positiveEnd) const {return positiveEnd?pVPos:pVNeg;}
	Vec3D<> force(bool positiveEnd) const {return positiveEnd?forcePos:forceNeg;}
	Vec3D<> moment(bool positiveEnd) const {return positiveEnd?momentPos:momentNeg;}
	float axialStrain() const {return 0.5*(strainNeg+strainPos);} //average is actually the exact answer
	float axialStrain(bool positiveEnd) const {return positiveEnd?strainPos:strainNeg;}

	bool isSmallAngle() const {return smallAngle;}

	bool isYielded() const;
	bool isYielded(bool positiveEnd) const;
	bool isFailed() const;
	bool isFailed(bool positiveEnd) const;

	float strainEnergy() const; //calculates the strain energy in the bond according to current forces and moments.
	float axialStiffness() const {return currentA1;} //returns the current axial stiffness of the bond
...
private:
	...
	float avgTransverseStrainSum();
	float avgTransverseArea();

	float strainNeg, strainPos; //axial strains (equal for homogneous bonds) :: totalstrain = (strainNeg+strainPos)/2.
	...
	float maxStrainNeg() const {return maxStrainRatio*(1+maxStrain)-1;}
	float maxStrainPos() const {return (1-maxStrainRatio)*(1+maxStrain)-1;}
	...
	float equalizeStress(float axialStrain, bool forceLinear=false); //the long (but thorough) way: updates strainNeg and strainPos according to the provided axial strain. returns current stress as well (MPa). forceLinear is used in conjunction with a modified strain value to account for plasticly deformed materials relaxed back into their linear region
	void scaleStrains(float axialStrain); //linearly scales strainNeg and strainPos to be consistent with axialStrain.


	typedef int linkState;
	enum linkFlags { //default of each should be zero for easy clearing
		HOMOGENOUS = 1<<0, //are both materials of the link equal?
		LINEAR = 1<<1, //are both material models linear?
		AREACONSTANT = 1<<2, //does either material have a non-zero poissons ratio?
		LOCAL_VELOCITY_VALID = 1<<3 //has something changes to render local velocity calculations (for damping) invalid? (this happens when small angle or global base size has changed)
	};
	linkState boolStates;			//single int to store many boolean state values as bit flags according to 
	
	bool isHomogenous() const {return boolStates & HOMOGENOUS;} //returns true if both materials in this bond are equal (many optimization if this is the case)
	bool isLinear() const {return boolStates & LINEAR;} //returns true if both materials are linear elastic
	bool isAreaConstant() const {return boolStates & AREACONSTANT;} //returns true if both materials have constant cross-sectional area (i.e. zero poissons ratio)
	bool isLocalVelocityValid() const {return boolStates & LOCAL_VELOCITY_VALID;} //returns true if both materials have constant cross-sectional area (i.e. zero poissons ratio)
	void setBoolState(linkFlags flag, bool set=true) {set ? boolStates |= flag : boolStates &= !flag;}


	void updateProperties(); //updates a1, b1, etc. based on the materials (and base size) of the linked voxels

	linkAxis axis;

	float E, currentA1; //current A1 will change with non-linear material and varying cross-sectional area
	float a1, a2, b1, b2, b3; //E is the composite elastic modulus for this bond
	float sqA1, sqA2xIp, sqB1, sqB2xFMp, sqB3xIp;

	Vec3D<double> pos2, angle1v, angle2v; //pos1 is always = 0,0,0
	CQuat<double> angle1, angle2; //this bond in local coordinates. 
	bool smallAngle; //based on compiled precision setting

	CQuat<> orientLink(float restLength); //updates pos2, angle1, angle2, and smallAngle. returns the rotation quaternion (after toAxisX) used to get to this orientation

	//unwind a coordinate as if the bond was in the the positive X direction (and back...)
	template <typename T> void toAxisX			(Vec3D<T>* const pV) const {switch (axis){case AXIS_Y: {T tmp = pV->x; pV->x=pV->y; pV->y = -tmp; break;} case AXIS_Z: {T tmp = pV->x; pV->x=pV->z; pV->z = -tmp; break;}}} //transforms a vec3D in the original orientation of the bond to that as if the bond was in +X direction
	template <typename T> void toAxisX			(CQuat<T>* const pQ) const {switch (axis){case AXIS_Y: {T tmp = pQ->x; pQ->x=pQ->y; pQ->y = -tmp; break;} case AXIS_Z: {T tmp = pQ->x; pQ->x=pQ->z; pQ->z = -tmp; break;}}}
	template <typename T> Vec3D<T> toAxisX		(const Vec3D<T>& v)	const {switch (axis){case AXIS_Y: return Vec3D<T>(v.y, -v.x, v.z); case AXIS_Z: return Vec3D<T>(v.z, v.y, -v.x); default: return v;}} //transforms a vec3D in the original orientation of the bond to that as if the bond was in +X direction
	template <typename T> CQuat<T> toAxisX		(const CQuat<T>& q)	const {switch (axis){case AXIS_Y: return CQuat<T>(q.w, q.y, -q.x, q.z); case AXIS_Z: return CQuat<T>(q.w, q.z, q.y, -q.x); default: return q;}} //transforms a vec3D in the original orientation of the bond to that as if the bond was in +X direction
	template <typename T> void toAxisOriginal	(Vec3D<T>* const pV) const {switch (axis){case AXIS_Y: {T tmp = pV->y; pV->y=pV->x; pV->x = -tmp; break;} case AXIS_Z: {T tmp = pV->z; pV->z=pV->x; pV->x = -tmp; break;}}}
	template <typename T> void toAxisOriginal	(CQuat<T>* const pQ) const {switch (axis){case AXIS_Y: {T tmp = pQ->y; pQ->y=pQ->x; pQ->x = -tmp; break;} case AXIS_Z: {T tmp = pQ->z; pQ->z=pQ->x; pQ->x = -tmp; break;}}}

...
public:
	...
	template <typename T> CQuat<T> toAxisX		(const CQuat<T>& q)	const {switch (ThisBondAxis){case AXIS_Y: return CQuat<T>(q.w, q.y, -q.x, q.z); case AXIS_Z: return CQuat<T>(q.w, q.z, q.y, -q.x); default: return q;}} //transforms a vec3D in the original orientation of the bond to that as if the bond was in +X direction
